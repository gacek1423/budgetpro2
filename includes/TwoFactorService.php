<?php
// includes/TwoFactorService.php

class TwoFactorService {
    private $_codeLength = 6;

    // 1. Generowanie sekretu (16 znaków Base32)
    ...
    // 3. Generowanie kodu QR (URL do zewnętrznego API np. goqr.me lub Google Charts)
    public function getQRCodeUrl($name, $secret, $title = null) {
        $urlencoded = urlencode('otpauth://totp/' . $name . '?secret=' . $secret . '');
        if (isset($title)) {
            $urlencoded .= urlencode('&issuer=' . urlencode($title));
        }
        // Używamy bezpiecznego API do generowania QR
